...
class CLIP_Clean_Train():
    def __init__(self, args):
        ...
        # Always-on CSV loss log (rank 0) so the loss curve can be plotted later WITHOUT needing
        # to have redirected stdout. Parsed by plot_losses.py --csv.
        self.loss_csv_path = os.path.join(self.logdir, "loss_log.csv") if self.is_main else None
        if self.loss_csv_path:
            with open(self.loss_csv_path, "w") as _f:
                _f.write("step,epoch,total_loss,L_long,L_pos,L_longer,L_hinge,logit_scale\n")
    ...
    def train_epoch(self, dataloader, epoch, start_iter=0, finegrained_loss=False, learnable_mps=True, naive_sampling=False):
        ...
        for i, (images, texts, short_text, _, _) in enumerate(tqdm(dataloader)):
            ...
            # Always-on CSV log (independent of --tee / stdout redirect).
            if self.is_main and self.loss_csv_path is not None and i % 50 == 0:
                with open(self.loss_csv_path, "a") as _f:
                    _f.write(f"{step},{epoch},{float(loss):.6f},{float(L_long):.6f},"
                             f"{float(L_pos):.6f},{float(L_longer):.6f},{float(L_hinge):.6f},"
                             f"{float(logit_scale):.4f}\n")

#!/bin/bash
# ============================================================
# Setup + eval 5-benchmark zero-shot (Flickr30K/COCO/Urban1K/SG4V-1K/DOCCI) TRÊN SERVER
# Paste toàn bộ vào terminal server, đứng đúng thư mục train/ (vd ~/Nhan_folder/train)
# Gồm: (1) ghi đè/tạo 7 file code, (2) tải data 3 benchmark, (3) build cache 5 benchmark,
#      (4) chạy eval sanity-check trên checkpoint RELEASED (chưa fine-tune).
# Sửa 3 biến dưới đây trước khi chạy nếu path khác:
# ============================================================
set -e

WORK_DIR=/cm/shared/chautvh_second/Nhan_folder/work
CKPT_DIR=/cm/shared/chautvh_second/Nhan_folder/ckpts
PROXY="http://<PROXY_USER>:<PROXY_PASS>@<PROXY_HOST>:<PROXY_PORT>"

export http_proxy="$PROXY"; export https_proxy="$PROXY"
export HTTP_PROXY="$PROXY"; export HTTPS_PROXY="$PROXY"

# ---------- (1) Ghi đè/tạo 7 file code ----------
...
echo '--- writing flickr_materialize.py ---'
cat > flickr_materialize.py << 'FLICKR_EOF_MARKER'
"""Materialize nlphuji/flickr30k (HuggingFace) into the CSV+jpg-folder layout
eval_paper_benchmarks.py / eval_zeroshot_retrieval.py expect:
  <root>/flickr_annotations_30k.csv   (columns: raw, sentids, split, filename, img_id)
  <root>/flickr30k-images/<filename>.jpg
Robust to a couple of schema variants since the exact HF dataset column names weren't verified
ahead of time (image field name, whether 'test' is a datasets-split or a column value)."""
import ast
import csv
import os
import sys
...
OUT_DIR = sys.argv[1] if len(sys.argv) > 1 else os.environ.get(
    "FLICKR_OUT", os.path.join(os.environ.get("EVAL_DATA_ROOT", "."), "flickr30k"))
IMG_DIR = os.path.join(OUT_DIR, "flickr30k-images")
os.makedirs(IMG_DIR, exist_ok=True)

print("[flickr30k] loading nlphuji/flickr30k ...")
try:
    ds = load_dataset("nlphuji/flickr30k", split="test")
    print(f"[flickr30k] loaded 'test' split directly, n={len(ds)}")
except Exception as e:
    print(f"[flickr30k] no literal 'test' split ({e}); loading full dataset and filtering by 'split' column")
    ds_full = load_dataset("nlphuji/flickr30k")
    key = list(ds_full.keys())[0]
    ds = ds_full[key].filter(lambda ex: ex.get("split") == "test")
    print(f"[flickr30k] filtered to split=='test', n={len(ds)}")

print("[flickr30k] columns:", ds.column_names)
...
for i, ex in enumerate(ds):
    ...
    if split != "test":
        continue
    filename = ex.get("filename") or f"{i}.jpg"
    img_id = ex.get("img_id", str(i))
    sentids = ex.get("sentids", "")
    caps = ex.get("caption") or ex.get("raw") or ex.get("captions") or ex.get("sentences")
    ...
    rows.append({"raw": repr(list(caps)), "sentids": sentids, "split": split,
                 "filename": filename, "img_id": img_id})
    if i % 200 == 0:
        print(f"[flickr30k] {i}/{len(ds)}")

csv_path = os.path.join(OUT_DIR, "flickr_annotations_30k.csv")
with open(csv_path, "w", newline="", encoding="utf8") as f:
    writer = csv.DictWriter(f, fieldnames=["raw", "sentids", "split", "filename", "img_id"])
    writer.writeheader()
    writer.writerows(rows)
print(f"[flickr30k] wrote {len(rows)} rows -> {csv_path}")
print(f"[flickr30k] images -> {IMG_DIR}")
FLICKR_EOF_MARKER

echo "=== all 8 file(s) written ==="

# ---------- (2) Tải data 3 benchmark còn thiếu ----------
mkdir -p "$WORK_DIR/eval_data/urban1k" "$WORK_DIR/eval_data/coco" "$WORK_DIR/eval_data/flickr30k"

echo "--- Urban1K ---"
if [ ! -d "$WORK_DIR/eval_data/urban1k/Urban1k" ]; then
  python -c "
...
"
  ...
  echo "[skip] Urban1K already present"
fi

echo "--- COCO val2017 ---"
if [ ! -f "$WORK_DIR/eval_data/coco/annotations/captions_val2017.json" ]; then
  ...
  echo "[skip] COCO already present"
fi

echo "--- Flickr30K (materialize via nlphuji/flickr30k) ---"
if [ ! -f "$WORK_DIR/eval_data/flickr30k/flickr_annotations_30k.csv" ]; then
  pip show datasets > /dev/null 2>&1 || pip install datasets
  FLICKR_OUT="$WORK_DIR/eval_data/flickr30k" python flickr_materialize.py "$WORK_DIR/eval_data/flickr30k"
else
  echo "[skip] Flickr30K already present"
fi

# ---------- (3) Build cache embedding cho 5 benchmark ----------
mkdir -p "$WORK_DIR/eval_data/eval_caches"

for ds in flickr30k coco urban1k; do
  if [ ! -f "$WORK_DIR/eval_data/eval_caches/$ds/part_00000.pt" ]; then
    echo "--- precompute cache: $ds ---"
    ...
    echo "[skip] cache $ds already present"
  fi
done

if [ ! -f "$WORK_DIR/eval_data/eval_caches/sg4v1k/part_00000.pt" ]; then
  echo "--- precompute cache: sg4v1k (full_json mode, server has original image tree) ---"
  ...
  echo "[skip] cache sg4v1k already present"
fi

if [ ! -e "$WORK_DIR/eval_data/eval_caches/docci" ]; then
  echo "!!! CHƯA symlink cache DOCCI. Nếu cache DOCCI training (part-file) đã có sẵn trên server,"
  echo "    chạy: ln -s <path_cache_docci_training_co_san> $WORK_DIR/eval_data/eval_caches/docci"
  echo "    rồi chạy lại script này (các bước trên đã [skip] nên không tốn công lặp lại)."
else
  echo "[ok] cache docci already linked"
fi

# ---------- (4) Sanity-check: eval checkpoint RELEASED (chưa fine-tune) ----------
echo "--- eval RELEASED (sanity check) ---"
...
echo "=== DONE. So kết quả eval_results/released_server.json với paper_reference_numbers.json ==="
echo "=== (nếu khớp sát -> harness đúng, sẵn sàng eval checkpoint vanilla/ours thật) ==="
